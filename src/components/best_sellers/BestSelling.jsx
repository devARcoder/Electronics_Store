  ...
  return (
    <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3 my-3'>
      {sellingData.map((sellItem, index)=> (
        <div className="flex flex-row items-center cursor-pointer border-gray-200 border-b border-r hover:shadow-xl my-3 md:my-5" key={index}>
          ...
          <h1 className='line-clamp-1 text-blue-600 text-md font-bold hover:text-yellow-400'>{sellItem.title}</h1>
            </div>
          <div className="flex space-x-2 items-center">
            <h1 className='text-gray-700 text-lg sm:text-xl md:text-xl'>{sellItem.price}</h1>
            <h1 className="line-through text-gray-400 text-lg sm:text-lg md:text-lg">{sellItem.oldPrice}</h1>
          </div>
          </div>
        </div>
      ))}
